...
base:
	! grep -P '\t' base.Dockerfile || { echo "ERROR: Tabs in base.Dockerfile" ; exit 1 ; }
...
standard:
	! grep -P '\t' standard.Dockerfile || { echo "ERROR: Tabs in standard.Dockerfile" ; exit 1 ; }
...
r:
	! grep -P '\t' r-ubuntu.Dockerfile || { echo "ERROR: Tabs in r-ubuntu.Dockerfile" ; exit 1 ; }
...
julia:
	! grep -P '\t' julia.Dockerfile || { echo "ERROR: Tabs in julia.Dockerfile" ; exit 1 ; }
	docker build -t aaltoscienceit/notebook-server-julia:$(VER_JULIA) --pull=false . -f julia.Dockerfile --build-arg=VER_BASE=$(VER_BASE)
opencv:
	! grep -P '\t' opencv.Dockerfile || { echo "ERROR: Tabs in opencv.Dockerfile" ; exit 1 ; }
